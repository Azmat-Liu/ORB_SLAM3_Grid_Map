...
int main(int argc, char **argv){
    ...
    node = rclcpp::Node::make_shared("Monosub");
	...
	rclcpp::Subscription<geometry_msgs::msg::PoseArray>::SharedPtr sub_pts_and_pose = node->create_subscription<geometry_msgs::msg::PoseArray>("pts_and_pose", 1000, ptCallback);
    rclcpp::Subscription<geometry_msgs::msg::PoseArray>::SharedPtr sub_all_kf_and_pts = node->create_subscription<geometry_msgs::msg::PoseArray>("all_kf_and_pts", 1000, loopClosingCallback);
    pub_grid_map = node->create_publisher<nav_msgs::msg::OccupancyGrid>("grid_map", 1000);
...
void cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr pt_cloud){
	RCLCPP_INFO(node->get_logger(),"I heard: [%s]{%d}", pt_cloud->header.frame_id.c_str(),
                pt_cloud->header.stamp);
}

void kfCallback(const geometry_msgs::msg::PoseStamped::SharedPtr camera_pose){
	RCLCPP_INFO(node->get_logger(),"I heard: [%s]{%d}", camera_pose->header.frame_id.c_str(),
...
//	}
    ...
    grid_map_msg.info.map_load_time = node->now();
...
void updateGridMap(const geometry_msgs::msg::PoseArray::SharedPtr pts_and_pose){
	...
	getGridMap();
        //showGridMap(pts_and_pose->header.stamp);
    showGridMap(pts_and_pose->header.stamp.sec+(pts_and_pose->header.stamp.nanosec * pow(10,-9)));
	//cout << endl << "Grid map saved!" << endl;
}

void resetGridMap(const geometry_msgs::msg::PoseArray::SharedPtr all_kf_and_pts){
...
//#endif
	...
	printf("Done. Time taken: %f secs\n", ttrack);
        //pub_grid_map.publish(grid_map_msg);
        pub_grid_map->publish(grid_map_msg);
        showGridMap(all_kf_and_pts->header.stamp.sec+(all_kf_and_pts->header.stamp.nanosec * pow(10,-9)));
}
